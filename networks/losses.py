...
class PhysicalLossV2(BaseLoss): # version 2: without darcy velocity and linear interpolation
    def __init__(self, device="cuda:2"):
        super().__init__(device)
        self.MSE = MSELoss()
        self.weight = [1.0, 1.0]
        self.mask = torch.tensor([[(i == 0) or (j == 0 or j == 15) for j in range(16)] for i in range(16)])
    ...
    def __call__(self, input, output, target, dataloader): # permeability index 1 of input, temperature index 1 and pressure index 0 of label
        ...
        permeability = nn.functional.pad(permeability, (1, 1, 1, 1), "replicate")
        pressure = inputs[1,:,:,:].unsqueeze(1)
        pressure = nn.functional.pad(pressure, (1, 1, 1, 1), "replicate")
        temperature_in = inputs[0,:,:,:].unsqueeze(1)
        temperature = norm.reverse(output_swap, "Labels").squeeze().unsqueeze(1)
        temperature = torch.cat([temperature_in[:, :, -1:, :], temperature], dim=2)
        # print(temperature_in[0,0], temperature_in[0, 0, -1:, :])
        # print(temperature.shape, temperature[0, 0])
        temperature = nn.functional.pad(temperature, (0, 0, 0, 1), "replicate")
        # print(temperature.shape, temperature[0, 0])
        temperature = self.constpad(temperature)
        # print(temperature.shape, temperature[0, 0])
        ...
        return physics_loss + 1e6 * boundary_error + 1e6 * temperature_error# + 1e6 * (torch.max(torch.abs(continuity_error_dx)) + torch.max(torch.abs(continuity_error_dy)) + torch.max(torch.abs(energy_error_dx)) + torch.max(torch.abs(energy_error_dy)))
...
class PhysicalLossV3(BaseLoss): # version 3: without darcy velocity and cubic interpolation
    def __init__(self, device="cuda:2"):
        super().__init__(device)
        self.MSE = MSELoss()
    ...
    def get_energy_error(self, temperature, pressure, permeability, cell_width):
        ...
        return energy_error_pressure + energy_error_temperature[..., 1:-1, 1:-1] # mistake around pump
    ...
    def complex_laplace(self, alpha: torch.tensor, values: torch.tensor, h = 1.0):
        ...
        result = torch.zeros(alpha.shape[:-2] + (N-4, M-4), device=alpha.device)
        for i in range(2, N-2):
            for j in range(2, M-2):
                alpha_i_p = -1/16 * alpha[..., i-1, j] + 9/16 * alpha[..., i  , j] + 9/16 * alpha[..., i+1, j] - 1/16 * alpha[..., i+2, j]
                alpha_i_m = -1/16 * alpha[..., i-2, j] + 9/16 * alpha[..., i-1, j] + 9/16 * alpha[..., i  , j] - 1/16 * alpha[..., i+1, j]
                alpha_j_p = -1/16 * alpha[..., i, j-1] + 9/16 * alpha[..., i, j  ] + 9/16 * alpha[..., i, j+1] - 1/16 * alpha[..., i, j+2]
                alpha_j_m = -1/16 * alpha[..., i, j-2] + 9/16 * alpha[..., i, j-1] + 9/16 * alpha[..., i, j  ] - 1/16 * alpha[..., i, j+1]
                val_left  = alpha_i_m  * values[..., i-1, j]
                val_right = alpha_i_p * values[..., i+1, j]
                val_top   = alpha_j_m * values[..., i, j-1]
                val_bott  = alpha_j_p * values[..., i, j+1]
