...
def preprocessing_allin1(args: argparse.Namespace, data_prep : Path):
    settings = deepcopy(args)
    destination = data_prep
    ...
    if (settings.dataset_prep / "Inputs" / prediction_destination).exists():
        ...
        model_1hp = load_1hp_model(args.device)
        model_ep = load_extend_model(args.device)
...
def load_1hp_model(device):
    ...
    model_1hp.load(args_1hpnn["model"], map_location=torch.device(device)) # TODO tut das auch für CPU? unnötig? torch.device weg?
    model_1hp.to(device)
    model_1hp.eval()
...
def load_extend_model(device):
    ...
    model_ep.load(args_extend2["model"], map_location=torch.device(device)) # TODO tut das auch für CPU? unnötig? torch.device weg?
    model_ep.to(device)
    model_ep.eval()
