...
def prepare_paths_and_settings(settings:SettingsTraining):
    if not settings.case_2hp:
        paths: Paths1HP
        paths, runs_problem_dir = set_paths_1hpnn(settings.dataset_raw, settings.inputs, settings.dataset_prep, problem=settings.problem) 
        settings.dataset_prep = paths.dataset_1st_prep_path
        inputs_2hp = None
    else:
        paths: Paths2HP
        inputs_2hp = settings.inputs
        paths, settings.inputs, runs_problem_dir = set_paths_2hpnn(settings.dataset_raw, settings.inputs, dataset_prep = settings.dataset_prep,)
        settings.dataset_prep = paths.datasets_boxes_prep_path

    settings.make_dirs(runs_problem_dir)
    settings.save_notes()
