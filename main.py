...
def run(settings: SettingsTraining):
    ...
    if settings.case in ["train", "finetune"]:
        ...
        try:
            solver.load_lr_schedule(os.path.join(os.getcwd(), "runs", settings.destination_dir, "learning_rate_history.csv"), settings.case_2hp)
            ...
            solver.save_lr_schedule(os.path.join(os.getcwd(), "runs", settings.destination_dir, "learning_rate_history.csv"))
    ...
    if settings.visualize:
        plot_sample(model, dataloaders["val"], settings.device, plot_name=settings.destination_dir + "/plot_val", amount_plots=5, pic_format="png")
...
if __name__ == "__main__":
    ...
    if not settings.case_2hp:
        ...
        #         default_raw_dir, settings.datasets_dir, dataset_prepared_full_path 
