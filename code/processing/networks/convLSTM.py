...
class ConvLSTMCell(nn.Module):
    def __init__(self, in_channels, out_channels, activation, frame_size, conv_features, kernel_sizes):
        ...
        layers.append(
            nn.Conv2d(
                in_channels=in_channels + conv_features[-1],
    ...
    def forward(self, X, H_prev, C_prev):
        ...
        # Rohit Panda adapted this idea from https://github.com/ndrplz/ConvLSTM_pytorch
        assert X.dim() == H_prev.dim(), f"X has shape {X.shape}, while H_prev has shape {H_prev.shape}"

        conv_output = self.conv[0](torch.cat([X, H_prev], dim=1))
        conv_output.to("cuda:0")
...
class ConvLSTM(nn.Module):
    def __init__(
        self, in_channels, out_channels, activation, frame_size, prev_boxes, extend, conv_features, kernel_sizes
    ):
        ...
        self.initial_H_projection = nn.Conv2d(
            in_channels=1,
        ...
        self.convLSTMcell = ConvLSTMCell(in_channels, out_channels, activation, frame_size, conv_features, kernel_sizes)
    ...
    def forward(self, X):
        ...
        # Initialize output
        output = torch.zeros(batch_size, self.out_channels, self.extend, height, width, device="cuda:0")

        # Split input into initial H and x
        H = X[:, -1, :, :, :].clone()
        X = X[:, :-1, :, :, :].clone()

        H = self.initial_H_projection(H)

        # Initialize Cell Input
        C = torch.zeros(batch_size, self.out_channels, height, width, device="cuda:0")

        # Unroll over time steps
        for time_step in range(self.prev_boxes, self.prev_boxes + self.extend):
            H, C = self.convLSTMcell(X.squeeze_(2), H, C)
            output[:, :, time_step] = H
...
class Seq2Seq(nn.Module):
    def __init__(
        ...
        self.sequential.add_module("batchnorm1", nn.BatchNorm3d(num_features=enc_conv_features[-1]))

        # Add rest of the ConvLSTM layers
        for layers in range(2, num_layers + 1):
            self.sequential.add_module(
                f"convlstm{layers}",
                ConvLSTM(
                    in_channels=enc_conv_features[-1],
                    out_channels=enc_conv_features[-1],
                    activation=activation,
                    frame_size=frame_size,
                    prev_boxes=prev_boxes,
            ...
            self.sequential.add_module(f"batchnorm{layers}", nn.BatchNorm3d(num_features=enc_conv_features[-1]))
    ...
    def forward(self, X):
        ...
        decoded_output = torch.zeros(batch_size, 1, self.extend, height, width, device="cuda:0")
        ...
        output = nn.Sigmoid()(decoded_output)
