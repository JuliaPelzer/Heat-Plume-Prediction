...
def build_streamlines(model_name:str, dataset_name:str="dataset_giant_100hp_varyK"):
    ...
    destination = data_dir/f"ds {origin_data_T_inout} prep_with_{model_path.name}"
    ...
    model = UNetNoPad2(in_channels=len(settings_model["inputs"]), out_channels=2, kernel_size=settings_model["kernel_size"], depth=settings_model["depth"], init_features=settings_model["init_features"], activation_fct=settings_model["activation"])
...
if __name__ == "__main__":
    ...
    model_name = "predict_v hyperparam_opt/best_predict_v_v4"
    build_streamlines(model_name, dataset_name)