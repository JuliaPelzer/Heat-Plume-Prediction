...
def init_data(settings: SettingsTraining, batch_size: int = 1000, labels: str = "t",):
    ...
    _do_assertions([reduce_to_2D, reduce_to_2D_xy], [batch_size], [
                   settings.dataset_name, settings.path_to_datasets])
    ...
    split = _split_test_data_extra_folder(
        split, settings.path_to_datasets, settings.dataset_name)
    ...
    for mode in modes:
        ...
        settings_dataset_temp = SettingsDataset(
            dataset_name=dataset_name_temp, input_vars_names=input_vars, output_vars_names=output_vars, mode=mode, split=split)
        temp_dataset = DatasetSimulationData(
            settings, settings_dataset_temp, transforms, means_stds_train_tuple,)
    ...
    for mode in modes:
        temp_dataloader = DataLoader(
            dataset=datasets[mode], batch_size=batch_size, shuffle=True, drop_last=False,)
    ...
    logging.warning(
        f'init done [total number of datapoints/runs: {np.sum([len(datasets[mode]) for mode in modes])}], with {len_datasets}')
...
def make_dataset_for_test(settings: SettingsTraining):
    ...
    settings_dataset_temp = SettingsDataset(
        dataset_name=settings.dataset_name, input_vars_names=input_vars, output_vars_names=output_vars, mode=mode, split=split)
    dataset = DatasetSimulationData(
        settings, settings_dataset_temp, transforms)
...
def _build_transforms(reduce_to_2D: bool, reduce_to_2D_xy: bool):
    # , PowerOfTwoTransform(oriented="left")]
    transforms_list = [ToTensorTransform()]
...
def _build_property_list(properties: str) -> List:
    ...
    for i in vars_list:
        assert i in ["x", "y", "z", "p", "t", "k", "i",
                     "s"], "input parameters have to be a string of characters, each of which is either x, y, z, p, t, k, i, s"
    ...
    if 'p' in vars_list:
        # unstructured grid, else Liquid_Pressure [Pa]
        vars.append("Liquid Pressure [Pa]")
    if 't' in vars_list:
...
def _split_test_data_extra_folder(split: dict, path_to_datasets: str, dataset_name: str):
    if split["test"] != 0:
        ...
        if not os.path.exists(f"{path_to_datasets}/{dataset_name}_TEST"):
            ...
            number_datapoints = len(os.listdir(
                f"{path_to_datasets}/{dataset_name}"))-1
            number_test_files = int(number_datapoints * split["test"])

            # select and move random datapoints/ files to test folder
            indices_test = np.random.permutation(number_datapoints)[
                :number_test_files]
            for index_file in indices_test:
                os.rename(
                    os.path.join(path_to_datasets, dataset_name,
                                 f"RUN_{index_file}"),
                    os.path.join(path_to_datasets,
                                 f"{dataset_name}_TEST", f"RUN_{index_file}")
                )

            # copy inputs folder
            os.mkdir(os.path.join(path_to_datasets,
                     f"{dataset_name}_TEST", "inputs"))
            for file in os.listdir(os.path.join(path_to_datasets, dataset_name, "inputs")):
                copyfile(os.path.join(path_to_datasets, dataset_name, "inputs", file),
                         os.path.join(path_to_datasets, f"{dataset_name}_TEST", "inputs", file))

        # if test folder already exists, use it
        else:
            logging.warning("Using existing TEST data folder")
