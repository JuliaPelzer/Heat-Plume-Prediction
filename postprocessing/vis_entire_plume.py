...
for dp in range(8):
    ...
    im2 = ax[2].imshow(error, cmap='coolwarm')

    cbar = fig.colorbar(im0, ax=ax[0], orientation='vertical')
    cbar = fig.colorbar(im0, ax=ax[1], orientation='vertical')
    cbar_error = fig.colorbar(im2, ax=ax[2], orientation='vertical')
    
    ax[0].set_title('Predicted Plume')
    ax[1].set_title('Label')
    ax[2].set_title('Error')
    
    plot_name = f'predicted_plume_steps{steps}_dp{dp}'
